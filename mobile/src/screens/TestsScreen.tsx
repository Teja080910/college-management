...
import { defaultTests } from '../utils/store'
import MaterialCommunityIcons from '@expo/vector-icons/MaterialCommunityIcons'

export default function TestsScreen() {
  const insets = useSafeAreaInsets()
  const sorted = [...defaultTests].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime())
  const now = Date.now()
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#f8fafc' }}>
      <ScrollView contentContainerStyle={{ paddingTop: insets.top + 16, padding: 16, paddingBottom: insets.bottom + 24 }}>
        ...
        <Text style={{ fontSize: 14, color: '#64748b', marginBottom: 16 }}>Upcoming examinations</Text>
