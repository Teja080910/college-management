      ...
      >
        ...
        <Card style={{ backgroundColor: '#ffffff', borderRadius: 16 }} elevation={1}>
          <Card.Content style={{ paddingVertical: 4 }}>
                ...
                <View style={{
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                    <View style={{
                      ...
                      backgroundColor: s.enrolled ? '#f0fdf4' : '#f1f5f9',
                    }}>
                      <Text style={{ fontSize: 12, fontWeight: '600', color: s.enrolled ? '#16a34a' : '#64748b' }}>
            ...
            {filtered.length === 0 && (
              <Text style={{ fontSize: 14, color: '#64748b', textAlign: 'center', paddingVertical: 24 }}>No students found</Text>
            )}
          </Card.Content>
        </Card>
      </ScrollView>
              ...
              <ScrollView keyboardShouldPersistTaps="handled" style={{ maxHeight: 500 }}>
                <View style={{ paddingHorizontal: 24, paddingTop: 12 }}>
                  ...
                  <Text style={{ fontSize: 14, color: '#64748b', marginBottom: 20 }}>
                    {editing ? 'Update the student details below.' : 'Fill in the details to register a new student.'}
                  </Text>

                  <TextInput label="Full name" value={form.name} onChangeText={t => setForm({ ...form, name: t })}
                  ...
                  <View style={{ flexDirection: 'row', gap: 10 }}>
                    <TextInput label="Age" value={form.age} onChangeText={t => setForm({ ...form, age: t })}
                      mode="outlined" keyboardType="numeric" style={{ marginBottom: 12, flex: 1, backgroundColor: '#f8fafc' }}
                      outlineStyle={{ borderRadius: 10, borderColor: '#e2e8f0' }} activeOutlineColor="#6366f1" />
                    <TextInput label="Grade" value={form.grade} onChangeText={t => setForm({ ...form, grade: t })}
                      mode="outlined" style={{ marginBottom: 12, flex: 1, backgroundColor: '#f8fafc' }}
                      outlineStyle={{ borderRadius: 10, borderColor: '#e2e8f0' }} activeOutlineColor="#6366f1" />
                  </View>
                  <TextInput label="Email" value={form.email} onChangeText={t => setForm({ ...form, email: t })}
                    mode="outlined" keyboardType="email-address" style={{ marginBottom: 12, backgroundColor: '#f8fafc' }}
                    outlineStyle={{ borderRadius: 10, borderColor: '#e2e8f0' }} activeOutlineColor="#6366f1" />
                  <TextInput label="Phone" value={form.phone} onChangeText={t => setForm({ ...form, phone: t })}
                    mode="outlined" style={{ marginBottom: 16, backgroundColor: '#f8fafc' }}
                    outlineStyle={{ borderRadius: 10, borderColor: '#e2e8f0' }} activeOutlineColor="#6366f1" />
                  ...
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 20 }}>
                    ...
                    <Chip selected={form.enrolled} onPress={() => setForm({ ...form, enrolled: true })}
                      style={{ backgroundColor: form.enrolled ? '#eef2ff' : '#f1f5f9' }}
                      textStyle={{ fontSize: 12, color: form.enrolled ? '#6366f1' : '#64748b' }}>Active</Chip>
                    <Chip selected={!form.enrolled} onPress={() => setForm({ ...form, enrolled: false })}
                      style={{ backgroundColor: !form.enrolled ? '#eef2ff' : '#f1f5f9' }}
                      textStyle={{ fontSize: 12, color: !form.enrolled ? '#6366f1' : '#64748b' }}>Inactive</Chip>
                  </View>
